...
internal static partial class Program
{
    private static void CheckPlanetaryProjection()
    {
        ...
        var batchPrices=inputs.ToDictionary(t=>t,t=>new PiQuote{Buy=90,Sell=100});
        batchPrices[robotics.Outputs.Keys.Single()]=new(){Buy=100000,Sell=110000};
        var tax=new PiTaxSettings{PocoPercent=10,SalesTaxPercent=7.5,SellImmediately=true};
        var batch=PlanetaryEconomics.Calculate(colony,robotics.Outputs.Keys.Single(),975,tax,batchPrices);
        Check(batch.Inputs.Count==4&&batch.Inputs.Values.All(v=>v==26000),"PI batch planner expands configured T3 chains into T1 inputs without taxing internal transfers");
        Check(batch.ImportTax==2080000&&batch.ExportTax==5850000&&batch.Net==71857500,"PI margin subtracts material replacement, half-value import duty, export duty and sale tax");
        Check(PlanetaryEconomics.Customs(1,1,10,false)==40&&PlanetaryEconomics.Customs(1,2,10,false)==720&&PlanetaryEconomics.Customs(1,4,10,true)==60000,"POCO fees use tier taxable values rather than market prices");
        tax.PocoPercent=null;
        Check(PlanetaryEconomics.Calculate(colony,robotics.Outputs.Keys.Single(),975,tax,batchPrices).Net==null,"Missing POCO rates do not silently become zero tax");
        tax.PocoPercent=10;batchPrices[inputs[0]].Sell=null;
        Check(PlanetaryEconomics.Calculate(colony,robotics.Outputs.Keys.Single(),975,tax,batchPrices).Net==null,"Missing input quotes leave PI profit pending");
        state.Taxes[PlanetaryEconomics.Key(colony)]=tax;
        var restoredTax=JsonSerializer.Deserialize<PiState>(JsonSerializer.Serialize(state))!;
        Check(restoredTax.Taxes[PlanetaryEconomics.Key(colony)].PocoPercent==10,"Manual POCO rates persist per pilot and planet");
